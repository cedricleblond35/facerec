...
def build_json(prediction, id_image, image):
	...
	for name, (top, right, bottom, left) in prediction:
		...
		personn_array.append(data_json)
	people_dictionary["Results"] = personn_array
	
	width, height = size_img(image)
	people_dictionary["Size"] = {"width" : width, "height": height}
...
if __name__ == "__main__":
	try:
		...
		#instancier la class Url
			...
			prediction = predict(image, model_path=trained_knn_model)
			predictions.append(build_json(prediction, id_image, image))
