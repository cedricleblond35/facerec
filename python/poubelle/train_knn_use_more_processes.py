...
def worker(list_directories):
	try:
		...
		Y = []
		unique_personn = 0
		for path_personn, file in list_directories.items():
			# Boucle à travers chaque image de formation pour la personne actuelle
			for img_path in image_files_in_folder(os.path.join(path_personn, file)):
...
def train(path_personn, model_save_path=None, n_neighbors=None, knn_algo='ball_tree', verbose=False):
	try:
		...
		for file in os.listdir(path_personn):
			if not os.path.isdir(os.path.join(path_personn, file)):
				continue
			
			
			personndict = {}
			personndict[path_personn] = file
		...
		result_worker = p.map(worker, list_directories)
		pool.close()
		pool.join()
		
		print(result_worker[302][1])
		i=0
		while i < len(result_worker):
			if(len(result_worker[i][1]) >0):
				Y.append(result_worker[i][1][0])
				X.append(result_worker[i][0])
			i += 1
		...
		if n_neighbors is None:
			n_neighbors = int(round(math.sqrt(len(result_worker[0]))))
			if verbose:
				print("Chose n_neighbors automatically:", n_neighbors)

		# Créer et former le classifieur KNN
		knn_clf = neighbors.KNeighborsClassifier(n_neighbors=n_neighbors, algorithm=knn_algo, weights='distance')
		knn_clf.fit(result_worker[0], result_worker[1])
